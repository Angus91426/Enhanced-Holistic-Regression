...
DETECTED_HEADERS = ['Method', 'Status', 'Detected_Relationship', 'Norm', 'Coefficient']
...
def run_detection_simulation(
    ...
    run_Enhanced_Corr: bool = True, run_Enhanced_Eigen: bool = True, run_Bertsimas: bool = True,
    ...
    log_path = os.path.join( OUTPUT_DIR, 'gurobi_live.log' )

    rng = np.random.default_rng( seed = seed )

    # ------------------------------------------------------------------ #
    # Ablation methods. Each is one fixed Multicollinear config; adjacent  #
    # comparisons differ by exactly one subprocess.                        #
    #   A : Correlation screen only                                        #
    #   B : Eigenvector screen only          (A->B isolates the screen)    #
    #   C : Inequality inspection only       (full matrix, no screen)      #
    #   D : Inequality + Irreducibility      (C->D isolates irreducibility)#
    #   E : Corr screen + Inequality + Irreducibility (no recovery)        #
    #   Enhanced Corr : E + Fast-path        (E->Enh isolates the fast-path)#
    ...
    METHODS = [
        ...
        { 'label': 'Method E', 'enabled': run_E, 'flags': {
            'reduction': True,  'reduction_method': 'corr', 'outlier_threshold': outlier_threshold,
            'Inequality_Inspection': True,  'Irreducibility_Inspection': True,  'fastpath': False } },
        { 'label': 'Enhanced Corr', 'enabled': run_Enhanced_Corr, 'flags': {
            'reduction': True,  'reduction_method': 'corr', 'outlier_threshold': outlier_threshold,
            'Inequality_Inspection': True,  'Irreducibility_Inspection': True,  'fastpath': True } },
        { 'label': 'Enhanced Eigen', 'enabled': run_Enhanced_Eigen, 'flags': {
            'reduction': True,  'reduction_method': 'eigvec',
            'Inequality_Inspection': True,  'Irreducibility_Inspection': True,  'fastpath': True } },
    ]
    ...
    for coef_min in coef_mins:
        ...
        for data_id, data_seed in enumerate( data_seeds ):
            ...
            for method in METHODS:
                ...
                trace_path = os.path.join( OUTPUT_DIR, f'Coef_min_{coef_min}_Sim_{data_id + 1}_{label}_Trace.csv' )
            ...
            if run_Bertsimas and coef_min == 0.0:
                ...
                multi_ = EHR.Multicollinear( norm_threshold = norm_threshold, alpha = alpha,
                                             total_time_limit = total_time_limit, per_solve_time_limit = per_solve_time_limit,
                                             log_file = log_path, log_tag = log_tag, trace = [] )
                start = time.time()
                ...
                # Bertsimas runs no inspection: blank Norm / Coefficient.
                for support in z_pos:
                    detected_rows.append( {
                        'Method': 'Original', 'Status': 'accepted',
                        'Detected_Relationship': _support_to_feature_str( support, feature_col ),
...
def run_realworld_detection(
    ...
    run_enhanced, run_bertsimas : bool
        Enable Enhanced_Detection and/or Bertsimas_Detection.
    reduction : bool
    ...
    def _detect( dName, X, feature_col, detector, trace_path ):
        ...
        start_ = time.time()
        if detector == 'Enhanced_Detection':
            z_pos = multi_.Enhanced_Detection( X = X, col_names = feature_col )
        else:
            z_pos = multi_.Bertsimas_Detection( X = X, col_names = feature_col )
    ...
    for dName in datasets:
        ...
        detectors = []
        if run_enhanced:  detectors.append( ( 'Enhanced_Detection', f'{dName}_Detected.csv' ) )
        ...
        for detector, trace_name in detectors:
            ...
            z_pos, ineq_fail, irre_fail, elapsed = _detect(
                dName, X, feature_col, detector, os.path.join( OUTPUT_DIR, trace_name ) )
